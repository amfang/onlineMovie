...
class ListView
{
	...
	$imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc',$flag='')
	{
	   ...
	   $flagsql = '';
		...
		{
			   $numquery = "Select id From `#@__archives` arc where {$this->addSql} $flagsql $ordersql";
          $this->dsql->SetQuery($numquery);
			...
			$t1 = ExecTime();
			$ids = array();
			$query = "Select id From `#@__archives` arc where {$this->addSql} $flagsql $ordersql limit $limitstart,$row ";
			...
			else
			{
				$query = "Select arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,
		   			...
		   			$addJoin
		   			where arc.id in($idstr) $flagsql $ordersql ";
			}
			$t2 = ExecTime();
			//echo $t2-$t1;
